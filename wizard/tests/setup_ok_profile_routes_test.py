...
def _build_app():
    state = {
        "profile": {
            "profile_name": "uDOS Local OK Profile",
            "user_quests": [],
            "local_skills": [],
            "knowledge_library": [],
        },
...
def test_ok_profile_template_and_get():
    app, _state = _build_app()
    client = TestClient(app)

    res = client.get("/api/setup/ok-profile/template")
    assert res.status_code == 200
    assert res.json()["template"]["schema_version"] == "1.3"

    res = client.get("/api/setup/ok-profile")
    assert res.status_code == 200
    assert res.json()["profile"]["profile_name"] == "uDOS Local OK Profile"


def test_ok_profile_mutation_routes():
    app, state = _build_app()
    client = TestClient(app)

    res = client.post("/api/setup/ok-profile", json={"profile": {"profile_name": "Custom"}})
    assert res.status_code == 200
    assert state["profile"]["profile_name"] == "Custom"
    assert state["marked"] == ["ok_profile"]

    res = client.post("/api/setup/ok-profile/quests", json={"quest": {"id": "q1"}})
    assert res.status_code == 200
    assert state["profile"]["user_quests"][0]["id"] == "q1"

    res = client.post("/api/setup/ok-profile/skills", json={"skill": {"id": "s1"}})
    assert res.status_code == 200
    assert state["profile"]["local_skills"][0]["id"] == "s1"

    res = client.post("/api/setup/ok-profile/knowledge", json={"entry": {"id": "k1"}})
    assert res.status_code == 200
    assert state["profile"]["knowledge_library"][0]["id"] == "k1"

    res = client.get("/api/setup/ok-profile/system-prompt?mode=coding")
    assert res.status_code == 200
