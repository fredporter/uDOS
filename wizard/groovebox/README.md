...
- Samples are downloaded locally (not committed) to `groovebox/sounds/`.
...
The script reads `sample-library.json`, fetches each file with `curl`, and writes it into `groovebox/sounds/<sample-id>`. The folder is gitignored so builds stay clean.
