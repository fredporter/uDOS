...
class PortManager:
    ...
    def check_service_port(self, service_name: str) -> ServiceStatus:
        ...
        if self.is_port_open(service.port):
            ...
            if occupant:
                service.status = ServiceStatus.RUNNING if occupant['process'] == service.process_name else ServiceStatus.PORT_CONFLICT
    ...
    def kill_service(self, service_name: str) -> bool:
        """Kill a service by name or port."""
        service = self.services.get(service_name)
        if not service or not service.port:
            return False
        ...
        try:
            subprocess.run(
                ['lsof', '-i', f':{service.port}', '|', 'grep', '-v', 'COMMAND', '|',
    ...
    def get_startup_order(self) -> List[str]:
        ...
        order = []
        for env in [ServiceEnvironment.PRODUCTION, ServiceEnvironment.DEVELOPMENT, ServiceEnvironment.EXPERIMENTAL]:
    ...
    def generate_report(self) -> str:
            ...
            if services:
                ...
                for svc in services:
                    status_icon = "✅" if svc.status == ServiceStatus.RUNNING else \
                                  "⚠️ " if svc.status == ServiceStatus.PORT_CONFLICT else \
                                  "❌" if svc.status == ServiceStatus.STOPPED else "❓"
                    
                    port_str = f":{svc.port}" if svc.port else "dynamic"
                    enabled_str = "✓" if svc.enabled else "✗"
                    
                    report.append(f"  {status_icon} [{enabled_str}] {svc.name:12} {port_str:8} | {svc.description}")
        ...
        if conflicts:
            ...
            for svc_name, occupant in conflicts:
                ...
                report.append(f"  Port {svc.port}: Expected {svc.name} but found {occupant['process']} (PID {occupant['pid']})")
            report.append("")
        ...
        for svc_name, occupant in conflicts:
            svc = self.services[svc_name]
            report.append(f"  Kill {occupant['process']}: lsof -i :{svc.port} | grep -v COMMAND | awk '{{print $2}}' | xargs kill -9")
