...
class WikiProvisioningService:
    ...
    def _generate_index_md(self) -> str:
        ...
        lines = ["# uDOS Wiki", "", self.structure.description, "", "---", ""]
        ...
        for category in sorted(self.structure.categories, key=lambda c: c.order):
            ...
            for page in sorted(category_pages, key=lambda p: p.order or 0):
                status_badge = {"draft": "📝", "published": "✅", "archived": "🗂️"}.get(
                    page.status, ""
                )
    ...
    def get_category(self, slug: str) -> WikiCategory | None:
...
def get_wiki_service(wiki_root: Path | None = None) -> WikiProvisioningService:
