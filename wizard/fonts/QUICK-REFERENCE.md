...
**Version:** 1.0.0 | **Location:** `/wizard/fonts/`
...
```
/wizard/fonts/          ← Central repository (source of truth)
...
```
1. Source: /wizard/fonts/      (Central repository)
           ↓
...
```bash
cp /path/to/newfont.ttf /wizard/fonts/bundled/category/
```
...
```bash
cd ~/uDOS/wizard/fonts && ./distribute.sh
```
