...
- `How do I...?` was treated as a raw command (not routed to OK ROUTE)
...
```
How do I create a file?        →  Routed to OK ROUTE (Vibe CLI)
What is the vault structure?   →  Routed to OK ROUTE (Vibe CLI)
```

**Implementation:** `_handle_question_mode()` routes to `OK ROUTE` dispatcher command for Vibe-powered natural language analysis.
...
2. **Wire up Goblin/Wizard Vibe endpoints** to the OK ROUTE handler so questions actually get answered.
...
3. **Test end-to-end:**
   ...
   ▶ How do I export files?   # Should route to OK ROUTE (if Wizard available)
   ```
