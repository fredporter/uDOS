...
- `tools/ci/check_spatial_seed_catalog.py` validates:
  - `core/src/spatial/anchors.default.json` anchor IDs/config shape
  - local `memory/spatial/anchors.json` and `memory/spatial/places.json` (when present) for placeRef syntax (`-Zz` optional), row/layer/z bounds, and anchor reference integrity
