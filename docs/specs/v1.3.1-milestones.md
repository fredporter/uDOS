# Roadmap Milestones (v1.3.1+)

This spec consolidates the milestone-level deliverables across uDOS. It replaces round/cycle framing with **Milestones** composed of **Moves** and **Steps**.

## Milestone: Wizard Web UI (Notion Blocks + Plugin Dashboard)
- **Owner:** Wizard
...
- **Next actions:** Maintain mapping persistence; keep plugin update scan flow stable; migrate overlay configs into plugin registry when ready.

## Milestone: Wizard Web Browser (Dataset Tables + SQLite Binding)
- **Owner:** Wizard (Dashboard)
- **Status:** ✅ Complete (schema/query + guardrails)
- **Deliverables:** Sortable/filterable dataset table, chart view, `/api/data/*` endpoints for tables/query/schema/export (pagination, filters like `filter=column:value`, ordering), spatial tagging filters, and teletext/Canvas endpoints so Svelte components can render 80×30 grids when data feeds `/api/teletext/*`.
- **References:** `docs/.archive/v1.2-earlier/file-parsing-architecture.md` (legacy), `specs/grid-spatial-computing.md`
- **Next actions:** Keep schema/query/export alignment; extend spatial filters when new datasets land.

## Milestone: Wizard Web Browser (Teletext Mode + NES Buttons)
- **Owner:** Wizard (Dashboard)
...
## Milestone: Beacon Portal + Sonic Device Catalog
...
- **Next actions:** Keep inventory of Goblin binder/mesh features and lock migration candidates.

## Milestone: Goblin Dev Server Experiments
- **Owner:** Goblin team
- **Status:** 🚧 In progress
...
## Milestone: Wizard Plugin Ecosystem + Bolt-On Distribution
...
## Milestone: App Typo Editor (Tauri) + Converters
- **Owner:** App team
...
- **References:** `sonic/docs/specs/sonic-stick-media-addon-brief.md`, `docs/specs/GROOVEBOX-SONGSCRIBE.md` and Songscribe syntax guide.
...
- **Owner:** Core
- **Status:** Active
- **Focus:** Keep Core TUI as the bedrock for future milestones by documenting hot-reload/self-heal habits, locking the new script executor guard and test suite, and maintaining story handler tests/fallbacks.
...
---
**Tracking note:** Keep this spec aligned with `docs/DEV-WORKFLOW-v1.3.1.md` and `docs/ROADMAP-TODO.md` so milestone progress stays consistent.
