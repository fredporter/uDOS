...
```
┌─────────────────────────────────────────────────────┐
│ SOURCE: Markdown Native (Flowcharts/Diagrams/Marp) │
│ (Version-controlled, human-readable, portable)     │
└────────────────────┬────────────────────────────────┘
                     │
        ┌────────────┴────────────┬─────────────┐
        │                         │             │
   ...
   └────┬─────┘         └────────┬──┘     └───┬────┘
        │                        │             │
        │ Fallback:              │ Primary:    │
        │ Teletext Blocks        │ SVG Proc.   │
        │ (ASCII-mapped)         │ (Renderer)  │
        │                        │             │
        └────────────┬───────────┴─────────────┘
                     │
          ┌──────────▼────────┐
...
```
knowledge/
├── food/recipe.md              # Inline Mermaid diagram
├── shelter/blueprint-guide.md  # Flowchart diagrams
└── diagrams/
    ├── firebow.txt            # ASCII result
    └── water-cycle.svg        # SVG result
```
...
- Box drawing characters (├─┬┤) form connections
- Block characters (▀▄▌▐█) create filled areas
...
**Pipeline:**
1. Markdown flowchart/diagram → Graphics Service
...
**Components:**
- [app/src/lib/components/DiagramRenderer.svelte](../../app/src/lib/components/DiagramRenderer.svelte) - SVG renderer
- [app/src/lib/components/MarpPresentation.svelte](../../app/src/lib/components/MarpPresentation.svelte) - Marp integration
...
│   ├─> SVG (via Graphics Service) → Tauri App
│   └─> ASCII (teletext mapped) → TUI
│
├─> Marp Presentation
│   ├─> HTML5 (via Marp CLI) → Export/Web
│   ├─> SVG (slides as images) → App
│   └─> Text outline → TUI (degraded)
│
└─> Knowledge Guide (inline)
    ├─> Extract diagrams
    ├─> Render each to ASCII → TUI
    └─> Render each to SVG → App
```
...
|---------|-----|----------------|
| Box | `<rect>` | `╔═══╗` |
| Arrow | `<path>` with arrowhead | `──→` or `─┬─` |
| Diamond (decision) | `<polygon>` | `◊` or `╱ ╲` |
| Circle | `<circle>` | `●` or `()`  |
...
### Phase 3: Markdown → Output Pipeline (2 weeks)
...
```
┌─────────────┬───────────────┬─────────────────┐
│ Color       │ Meaning       │ Teletext Block  │
├─────────────┼───────────────┼─────────────────┤
│ Green       │ Success/OK    │ ✓ or ◆ (filled) │
│ Red         │ Error/Fail    │ ✗ or ▲ (filled) │
│ Blue        │ Process       │ ■ (box)         │
│ Yellow      │ Decision      │ ◇ (diamond)     │
│ Cyan        │ Input/Output  │ ◯ (circle)      │
│ Magenta     │ Action        │ ★ (star)        │
│ Default     │ Neutral       │ Normal          │
└─────────────┴───────────────┴─────────────────┘
```
...
✅ **Graphics Tier 1 (Markdown Source):**
...
✅ **Graphics Tier 2 (ASCII-Teletext):**
