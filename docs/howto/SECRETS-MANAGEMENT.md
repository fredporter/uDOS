...
**Status:** 🔐 All API keys moved to `.env` (gitignored)  
...
```
wizard/config/ai_keys.json        ← Contains LIVE OpenAI/Gemini keys in git
public/wizard/config/ai_keys.json ← Exposed to public repo
```
...
.env.template                     ← Shared template (no secrets, committed to git)
wizard/config/ai_keys.json        ← Generated from .env (gitignored)
```
...
Done! ✅ Config files generated automatically.
...
- ✅ `.env.template` is committed (template only, no secrets)
- ✅ Generated config files are gitignored
- ✅ Never share `.env` file or commit it
- ✅ Use environment-specific `.env.local` files for sensitive overrides
