...
- Public: fredporter/uDOS (release docs + assets mirror)
...
# Clone public
rm -rf /tmp/uDOS
git clone https://github.com/fredporter/uDOS.git /tmp/uDOS

# Copy source code
rsync -av --delete --exclude='__pycache__' --exclude='*.pyc' --exclude='tests/' --exclude='.dev' --exclude='.archive' core/ /tmp/uDOS/core/
rsync -av --delete --exclude='__pycache__' --exclude='*.pyc' --exclude='tests/' --exclude='.dev' --exclude='.archive' extensions/api/ /tmp/uDOS/extensions/api/

# Copy distribution and tinycore tools
rsync -av --delete --exclude='*.iso' --exclude='builds/' --exclude='test/' distribution/ /tmp/uDOS/distribution/
rsync -av --delete --exclude='__pycache__' --exclude='*.pyc' --exclude='*.iso' library/tinycore/ /tmp/uDOS/library/tinycore/

# Copy data and knowledge (core/ already includes core/docs/)
rsync -av --delete data/ /tmp/uDOS/data/
rsync -av --delete knowledge/ /tmp/uDOS/knowledge/

# Generate README (or copy pre-generated one)
rsync -av LICENSE.txt /tmp/uDOS/
...
- Create `fredporter/uDOS` (empty on `main`)
