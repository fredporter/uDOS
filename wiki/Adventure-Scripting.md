...
Complete guide to creating interactive text adventures in uDOS using `.upy` (uPython Adventure) scripts.
...
# sandbox/ucode/adventures/my_adventure.upy
...
# Numeric comparison
IF [$SPRITE-HP < 30]
  PRINT [You're badly wounded!]
  HP [+10]  # Emergency healing
ENDIF

# Greater than or equal
IF [$SPRITE-LEVEL >= 5]
  PRINT [You're strong enough to enter!]
  BRANCH [DUNGEON]
ENDIF

# String comparison
IF [$SPRITE-NAME == "Alice"]
  PRINT [Hello, Alice!]
ENDIF
...
```python
# Works but complex - use branching instead
IF [$SPRITE-LEVEL >= 3]
  IF [$SPRITE-GOLD >= 100]
    PRINT [You can afford the upgrade!]
  ENDIF
ENDIF
```
...
# Use in gameplay
ROLL [1d20] → $ATTACK-ROLL
IF [$ATTACK-ROLL >= $SPRITE-DEXTERITY]
...
# Check progress
IF [$STORY-CHAPTER >= 3]
  PRINT [You've come far in your journey...]
ENDIF
```
...
IF [$STORY-FLAGS-contains-"saved_village"]
  BRANCH [ENDING-HERO]
ENDIF

IF [$SPRITE-LEVEL >= 10]
  BRANCH [ENDING-CHAMPION]
ENDIF
...
ENDIF
IF [$SEARCH < 12]
  PRINT [No water found. Dehydration worsens.]
  HP [-10]
ENDIF
END
...
IF [$LOCKPICK-ROLL >= 12]
  PRINT [Success! The lock clicks open.]
  ITEM [treasure]
  XP [+60]
  FLAG [picked_lock]
ENDIF
...
IF [$RANDOM-EVENT <= 20]
  PRINT [A merchant appears!]
  BRANCH [EVENT-MERCHANT]
ENDIF
