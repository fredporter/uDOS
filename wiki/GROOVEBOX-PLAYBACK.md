...
- Public-domain samples live in `wizard/groovebox/sample-library.json`; run `bash wizard/groovebox/setup-samples.sh` to download them into `groovebox/sounds/`. The folder is gitignored so each dev can refresh the samples locally without polluting the repo.
