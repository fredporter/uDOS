"""
Pixel-to-Sextant Graphics Lookup Tables
========================================

Comprehensive lookup tables for converting pixel grids (2×3, 4×6, etc.) to
Unicode sextant characters with fallback chains.
...
# =============================================================================
# SEXTANT CHARACTER ENCODING
# =============================================================================

class SextantMask(IntEnum):
    """6-bit sextant pixel positions (2×3 grid)."""
    TOP_LEFT = 1      # 0b000001
...
# Unicode Sextant Block Characters (2×3 pixel grid)
# Mapping from 6-bit pattern (0-63) to sextant character
SEXTANT_CHARS = {
    0b000000: " ",   # Empty
...
# =============================================================================
# QUADRANT CHARACTER ENCODING (FALLBACK 1)
# =============================================================================

class QuadrantMask(IntEnum):
    """4-bit quadrant pixel positions (2×2 grid)."""
    TOP_LEFT = 1     # 0b0001
    TOP_RIGHT = 2    # 0b0010
    BOT_LEFT = 4     # 0b0100
    BOT_RIGHT = 8    # 0b1000
...
def sextant_to_quadrant(sextant_mask: int) -> int:
    """
    Convert 6-bit sextant pattern (2×3) to 4-bit quadrant pattern (2×2).
    Strategy: average each quadrant's pixels.
    
    Sextant layout:        Quadrant layout:
      0 1                    0 1
      2 3                    2 3
      4 5
    """
    # Map sextant pixel positions to quadrant
    # TL: sextant 0 only
    # TR: sextant 1 only
    # BL: sextant 2,4 average
    # BR: sextant 3,5 average
    
    tl = (sextant_mask & 1) >> 0
    tr = (sextant_mask & 2) >> 1
    bl = ((sextant_mask & 4) >> 2) | ((sextant_mask & 16) >> 4)  # avg 2,4
    br = ((sextant_mask & 8) >> 3) | ((sextant_mask & 32) >> 5)  # avg 3,5
    
    return (bl & 1) | ((tr & 1) << 1) | ((bl & 1) << 2) | ((br & 1) << 3)


def quadrant_to_shade(quadrant_mask: int) -> int:
    """
    Convert 4-bit quadrant pattern to 1-4 shade level.
    Count filled quadrants: 0=empty, 4=full.
    """
    return bin(quadrant_mask).count('1')


def sextant_to_shade(sextant_mask: int) -> int:
    """
    Convert 6-bit sextant pattern directly to shade level.
    Simpler: just count pixels.
    """
    return bin(sextant_mask).count('1')
...
def pixel_grid_to_sextant(grid_2x3: List[List[int]]) -> str:
    """
    Convert a 2×3 pixel grid to a sextant character.
    
    Args:
        grid_2x3: 2D array of 2×3 (rows=2, cols=3) with values 0 or 1
    
    Returns:
        Sextant Unicode character
    ...
    Example:
        ...
        ch = pixel_grid_to_sextant(grid)  # Returns sextant character
    """
    ...
    return SEXTANT_CHARS.get(pattern, " ")


def pixel_grid_to_fallback(grid_2x3: List[List[int]], style: str = "quadrant") -> str:
    """
    Convert pixel grid to fallback character (non-sextant).
    
    Args:
        grid_2x3: 2D pixel grid (2×3)
        style: "quadrant", "shade", or "ascii"
    ...
    if style == "shade":
        ...
        # Convert to 4-bit quadrant pattern
        pattern = sextant_to_quadrant(
            sum(1 << i for i, cell in enumerate([
                grid_2x3[0][0], grid_2x3[0][1:3],
                grid_2x3[1][0], grid_2x3[1][1:3]
            ]) if any(cell) if isinstance(cell, int) else cell)
        )
        return QUADRANT_CHARS.get(pattern, " ")
    
    else:
        return " "


def get_fallback_chain(sextant_mask: int) -> Dict[str, str]:
    """
    Get all fallback options for a sextant pattern.
    Returns dict with keys: "sextant", "quadrant", "shade", "ascii".
    """
    return {
        "sextant": SEXTANT_CHARS.get(sextant_mask, " "),
        "quadrant": QUADRANT_CHARS.get(sextant_to_quadrant(sextant_mask), " "),
        "shade": SHADE_CHARS.get(sextant_to_shade(sextant_mask), " "),
        "ascii": ASCII_FALLBACK.get(sextant_to_shade(sextant_mask), " "),
    }


def choose_character(sextant_mask: int, style: str) -> str:
    """
    Choose character based on style preference.
    Falls back if style is unsupported.
    
    Args:
        sextant_mask: 6-bit pattern (0-63)
        style: "sextant", "quadrant", "shade", or "ascii"
    
    Returns:
        Best available character for the style
    """
    fallback = get_fallback_chain(sextant_mask)
    
    if style in fallback:
        ch = fallback[style]
        return ch if ch != " " else fallback.get("ascii", " ")
    
    # Fallback chain: sextant → quadrant → shade → ascii
    for key in ["sextant", "quadrant", "shade", "ascii"]:
        if fallback[key] != " ":
            return fallback[key]
...
def pixels_to_string(
    ...
    Args:
        ...
        style: "sextant", "quadrant", "shade", "ascii"
    ...
    Example:
        ...
        output = pixels_to_string(pixels, style="sextant")
        print(output)
    """
    ...
    # Process in 2×3 chunks (sextant size)
    for y in range(0, len(pixel_array), 2):
        ...
        for x in range(0, len(pixel_array[0]), 3):
            ...
            # Get sextant character
            try:
                ch = pixel_grid_to_sextant(grid) if style == "sextant" else pixel_grid_to_fallback(grid, style)
                row_chars.append(ch)
            except Exception:
                row_chars.append(" ")
...
if __name__ == "__main__":
    print("🎨 Pixel-to-Sextant Lookup Tables v1.0.0")
    print("=" * 60)
    
    # Test 1: Simple sextant patterns
    print("\n📊 Sextant Character Samples:")
    patterns = [
        (0b000000, "Empty"),
        (0b111111, "Full"),
    ...
    print("\n🖼️  Pixel Grid → Sextant:")
    grid = [
        [1, 1, 0],
        [1, 0, 1]
    ]
    ch = pixel_grid_to_sextant(grid)
    print(f"  Grid pattern → {ch}")
    
    # Test 4: Full tile rendering
    print("\n🎯 Full 16×24 Tile Rendering (sextant):")
    pixels = [[0] * 16 for _ in range(24)]
    # Draw a circle
    for y in range(24):
        for x in range(16):
