...
def run_script(script_path, parser, grid, command_handler, logger, command_history=None):
    ...
    try:
        ...
        logger.log(f"🔷 Running uPY v1.2.x script: {script_file}")
        ...
        while True:
            try:
                ...
                if tui and tui.mode == "ok_panel":
                    ...
                    if panel_input.strip().upper() in ('ESC', 'ESCAPE', 'EXIT', 'CLOSE'):
                        tui.close_ok_panel()
                        ...
                        # Execute OK command
                        result = command_handler(panel_input, context={'source': 'ok_panel'})
                        print(result)
                ...
                if tui and tui.mode == "browser":
                    ...
                    if browser_action and browser_action.get('action') == 'show_file_menu':
                        ...
                        if op_input.upper() == 'O':
                            ...
                            result = tui.perform_file_operation('copy', destination=Path(dest))
                            ...
                            result = tui.perform_file_operation('move', destination=Path(dest))
                            if result.get('success'):
                            ...
                            if result.get('needs_confirmation'):
                                confirm = smart_prompt.ask(prompt_text=f"Delete {result['name']}? (yes/no)> ")
                                if confirm.lower() == 'yes':
                                    result = tui.perform_file_operation('delete', confirm=True)
                                    if result.get('success'):
