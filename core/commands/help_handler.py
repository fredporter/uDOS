...
class HelpHandler:
    ...
    def _show_command_list(self) -> str:
        ...
        lines.append(f"  {c['option']}Variables:{c['reset']}         {{$hp}}, {{$gold}}, {{$MISSION.STATUS}}")
        lines.append(f"  {c['option']}Commands:{c['reset']}          PRINT[ Hello World ]")
        lines.append(f"  {c['option']}Arguments:{c['reset']}         PRINT[ arg1 | arg2 | arg3 ]")
