...
class SystemCommandHandler(BaseCommandHandler):
    ...
    def handle(self, command, params, grid, parser):
        ...
        handlers = {
            ...
            'GHOST': self.handle_ghost_mode,
            'TOMB': self.handle_tomb_mode,
            'CRYPT': self.handle_crypt_mode,
        }
    ...
    def handle_mode(self, params, grid, parser):
        ...
        if subcommand == 'STATUS':
            output = []
            output.append("🎭 Available Prompt Modes:")
            output.append("")
            output.append("🌀 COMMAND - Standard operations (default)")
            output.append("⚙️ DEV     - Developer/admin access")
            output.append("❤️ ASSIST  - AI-powered assistance")
            output.append("👻 GHOST   - Demo/sandbox/offline-testing")
            output.append("🔒 TOMB    - Archival local/private only")
            output.append("🛜 CRYPT   - Archival with networking")
            output.append("")
        ...
        mode_descriptions = {
            ...
            'dev': '⚙️ DEV MODE - Developer/admin access',
            'assist': '❤️ ASSIST MODE - AI-powered assistance',
            'ghost': '👻 GHOST MODE - Demo/sandbox/offline-testing',
            'tomb': '🔒 TOMB MODE - Archival local/private only',
            'crypt': '🛜 CRYPT MODE - Archival with networking'
        }
        
        return f"✅ Switched to {mode_descriptions[new_mode]}"

    def handle_ghost_mode(self, params, grid, parser):
        """Toggle GHOST mode (demo/sandbox/offline-testing).
        ...
        if current_mode == 'ghost':
            ...
            output.append("  • Isolated testing environment")
            output.append("  • No persistent file changes")
            output.append("  • Network disabled")
            output.append("  • Safe experimentation")
            output.append("")
            output.append("💡 Run GHOST again to exit")
            return "\n".join(output)

    def handle_tomb_mode(self, params, grid, parser):
        """Toggle TOMB mode (archival local/private only).
        ...
        if current_mode == 'tomb':
            ...
            output.append("")
            output.append("Features:")
            output.append("  • Read-only archival access")
            output.append("  • Local storage only")
            output.append("  • No network access")
            output.append("  • No sharing capabilities")
            output.append("")
            output.append("💡 Run TOMB again to exit")
            return "\n".join(output)

    def handle_crypt_mode(self, params, grid, parser):
        """Toggle CRYPT mode (archival with networking).
        ...
        if current_mode == 'crypt':
            ...
            output.append("")
            output.append("Features:")
            output.append("  • Read-write archival access")
            output.append("  • Network enabled for sync")
            output.append("  • Location/beacon/key sharing")
            output.append("  • Collaborative archival work")
            output.append("")
            output.append("💡 Run CRYPT again to exit")
            return "\n".join(output)
