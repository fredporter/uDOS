    ...
    def _test_planet_system(self, output: List[str], verbose: bool):
        ...
        if exists:
            try:
                ...
                self._add_test("Planet: universe.json valid JSON", False, str(e))        # Check planets.json (DEPRECATED - v1.2.12, now optional)
...
"""
        ...
        symbol = "✅" if exists else "⚠️"
        if verbose or not exists:
            output.append(f"  {symbol} debug_test.upy {'exists' if exists else 'optional'}")
    ...
    def _test_memory_structure(self, output: List[str], verbose: bool):
        ...
        for dir_name in workflow_dirs:
            ...
            self._add_test(f"Workflows: {dir_name} directory", True)  # May not exist yet
    ...
    def _test_api_monitoring(self, output: List[str], verbose: bool):
        ...
        try:
            ...
            output.append(f"  {symbol} Workflow variables (PROMPT.*, GENERATE.*, API.*) - optional")
            self._add_test("API: workflow variables", True)  # Optional feature, always pass
    ...
    def _test_hot_reload(self, output: List[str], verbose: bool):
        ...
        try:
            ...
            if isinstance(result, ReloadResult) and not result.success:
                ...
                self._add_test("Hot Reload: error handling", True)  # Non-critical, always pass
    ...
    def _test_folder_structure(self, output: List[str], verbose: bool):
        ...
        required_structure = {
            'memory/ucode/scripts': {'type': 'dir', 'tracked': False, 'description': 'User .upy scripts'},
            'memory/ucode/tests': {'type': 'dir', 'tracked': True, 'description': 'Test suites'},
            'memory/ucode/sandbox': {'type': 'dir', 'tracked': False, 'description': 'Working/experimental scripts'},
            'memory/ucode/stdlib': {'type': 'dir', 'tracked': True, 'description': 'Standard library'},
            'memory/ucode/examples': {'type': 'dir', 'tracked': True, 'description': 'Example scripts'},
            str(PATHS.MEMORY_UCODE_ADVENTURES): {'type': 'dir', 'tracked': True, 'description': 'Adventure scripts'},
            str(PATHS.MEMORY_WORKFLOWS_MISSIONS): {'type': 'dir', 'tracked': False, 'description': 'Mission scripts'},
            str(PATHS.MEMORY_WORKFLOWS_CHECKPOINTS): {'type': 'dir', 'tracked': False, 'description': 'State snapshots'},
            str(PATHS.MEMORY_WORKFLOWS_STATE): {'type': 'dir', 'tracked': False, 'description': 'Current execution state'},
            'memory/workflows/extensions': {'type': 'dir', 'tracked': False, 'description': 'Gameplay integration'},
            'memory/system/user': {'type': 'dir', 'tracked': False, 'description': 'User settings'},
            'memory/system/themes': {'type': 'dir', 'tracked': False, 'description': 'Custom themes'},
            'memory/bank': {'type': 'dir', 'tracked': False, 'description': 'Banking/transactions'},
            'memory/shared': {'type': 'dir', 'tracked': False, 'description': 'Shared/community content'},
            'memory/docs': {'type': 'dir', 'tracked': False, 'description': 'User documentation'},
            'memory/drafts': {'type': 'dir', 'tracked': False, 'description': 'Draft content'},
        }
        ...
        try:
            ...
            if not missing:
                output.append(f"  ✅ All required folders present ({len(present)}/{len(required_structure)})")
                if verbose:
                    for folder in sorted(present):
                        output.append(f"      ✓ {folder}")
                self._add_test("Folder Structure: required folders", True)
            else:
                output.append(f"  ⚠️  Some folders missing ({len(present)}/{len(required_structure)})")
                if verbose:
                    output.append(f"      Missing folders:")
                    for folder in sorted(missing):
                        desc = required_structure[folder]['description']
                        output.append(f"        ❌ {folder} ({desc})")
                self._add_test("Folder Structure: required folders", len(missing) <= 2)  # Allow 2 missing
            ...
            if not deprecated_found:
                output.append(f"  ✅ No deprecated root folders ({len(deprecated_clean)}/{len(deprecated_folders)} archived)")
                self._add_test("Folder Structure: deprecated cleanup", True)
            else:
                output.append(f"  ❌ Deprecated folders still present")
                if verbose:
                    for folder in deprecated_found:
                        output.append(f"      ❌ {folder}/ should be archived")
                        output.append(f"         {deprecated_folders[folder]}")
                self._add_test("Folder Structure: deprecated cleanup", False)
            ...
            # Test 5: Verify test files in correct location
            ucode_tests = self.root / 'memory' / 'ucode' / 'tests'
            if ucode_tests.exists():
                test_files = list(ucode_tests.glob('test_*.py'))
                if test_files:
                    output.append(f"  ✅ Test files in memory/ucode/tests/ ({len(test_files)} files)")
                    self._add_test("Folder Structure: test location", True)
                else:
                    output.append(f"  ⚠️  No test files found in memory/ucode/tests/")
                    self._add_test("Folder Structure: test location", False)
            else:
                output.append(f"  ❌ memory/ucode/tests/ not found")
                self._add_test("Folder Structure: test location", False)
