...
from extensions.play.screwdriver_flash_packs import (
    FlashPackManager,
    FlashPackStatus,
    FlashStage
)
from extensions.play.screwdriver_provisioner import (
...
class ScrewdriverCommandHandler:
    ...
    def _handle_flash(self, args: List[str]) -> str:
        ...
        # Show current state
        lines.append(f"Current: v{device.firmware_version} {device.firmware_status.value}")
    ...
    def _show_tile_health(self, tile: str) -> str:
        ...
        for device in devices:
            ...
            lines.append(f"{status_icon} {device.id:<8} {device.type.value:<12} {health.signal_strength:>3}% signal")
        
        lines.append("")
        lines.append(f"Summary: {healthy_count} healthy, {warn_count} warnings, {fail_count} failures")
    ...
    def _handle_provision(self, args: List[str]) -> str:
        ...
        if len(args) < 2:
            return ("Usage: SCREWDRIVER PROVISION <devices> <version> [--strategy=<strategy>]\n"
        ...
        if "," in device_spec:
            ...
            device_ids = [
                d.id for d in self.device_manager.devices.values()
        ...
        job = self.provisioner.create_provision_job(
            device_ids,
            version,
            strategy
        )
    ...
    def _show_all_status(self) -> str:
        ...
        lines.append(f"{'Device':<10} {'Type':<12} {'Version':<10} {'Status':<8} {'Signal':<8}")
        lines.append("-" * 60)
        ...
        if latest_version:
            ...
            lines.append("")
            lines.append(f"Latest: v{latest_version} ({up_to_date}/{len(devices)} devices up to date)")
    ...
    def _show_device_status(self, device_id: str) -> str:
        ...
        lines.append(f"  Status: {device.firmware_status.value} {device.firmware_status.name}")
    ...
    def _list_jobs(self) -> str:
        ...
        lines.append(f"{'Job ID':<15} {'Status':<12} {'Devices':<10} {'Success':<10} {'Failures':<10}")
        lines.append("-" * 70)
...
def demo_screwdriver_commands():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        handler.device_manager.register_device("D1", "AA340", 600, DeviceType.NODE, "2.4.0")
        handler.device_manager.register_device("D2", "AA340", 600, DeviceType.GATEWAY, "2.4.0")
