...
class ServerManager:
    ...
    def _use_bulletproof_launcher(self, server_name: str, port: Optional[int] = None,
        ...
        try:
            ...
            log_dir = Path('memory/logs/servers')
    ...
    def list_servers(self) -> str:
        ...
        # Check cmd
        cmd_dir = self.web_dir / 'cmd'
        if (cmd_dir / 'package.json').exists():
            status = "✅ Installed"
            if 'cmd' in self.servers and self._is_process_running(self.servers['cmd'].get('pid')):
                status += f" (Running on port {self.servers['cmd'].get('port', 3000)})"
        else:
            status = "❌ Not installed"
        lines.append(f"cmd             - Web terminal emulator - {status}")

        # Check markdown-viewer
        viewer_dir = self.extensions_dir / 'web' / 'markdown-viewer'
        if (viewer_dir / 'server.py').exists():
            status = "✅ Installed"
            if 'markdown-viewer' in self.servers and self._is_process_running(self.servers['markdown-viewer'].get('pid')):
                status += f" (Running on port {self.servers['markdown-viewer'].get('port', 8889)})"
        else:
            status = "❌ Not installed"
        lines.append(f"markdown-viewer - GitHub-style MD renderer - {status}")

        # Check font-editor
        font_dir = self.extensions_dir / 'web' / 'font-editor'
        if (font_dir / 'index.html').exists():
            status = "✅ Installed"
            if 'font-editor' in self.servers and self._is_process_running(self.servers['font-editor'].get('pid')):
                status += f" (Running on port {self.servers['font-editor'].get('port', 8888)})"
        else:
            status = "❌ Not installed"
        lines.append(f"font-editor     - Bitmap font editor - {status}")

        # Check dashboard
        dashboard_dir = self.extensions_dir / 'web' / 'dashboard'
        ...
        lines.append("  typo:      cd extensions && bash setup_typo.sh")
        lines.append("  cmd:       bash extensions/setup_cmd.sh")
        lines.append("  markdown-viewer, font-editor, and dashboard are pre-installed")
        lines.append("")
        lines.append("Usage:")
