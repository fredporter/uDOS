...
- **todo.checklist** – Short-term actionable tasks. Maps to `TodoManager`, Notion `to_do` blocks, calendar/Gantt renders, and triggers the todo reminder service.
...
- `uCODETUI._cmd_prompt` (added in `core/tui/ucode.py`) consumes `get_prompt_parser_service()`, adds the parsed tasks to `TodoManager`, renders the 80×30 calendar/Gantt previews, and prints Notion block payloads.  
- `wizard/routes/task_routes.parse_prompt` exposes `/api/tasks/prompt`; it returns the parsed instruction metadata, Notion block list, calendar/Gantt lines (text + JSON options), and the reminder payload emitted by `TodoReminderService`.
