...
1. **font-system.json** (`core/data/font-system.json`)
...
**`core/data/font-system.json`** (400+ lines)
```json
{
  "color_palette": {
...
**`core/data/font-profile-template.json`** (200+ lines)
```json
{
  "metadata": {
...
profile = settings.load_user_font_profile('sandbox/user/my-profile.json')
```
...
async loadFontSystemConfig() {
    const response = await fetch('../../core/data/font-system.json');
    this.fontSystemConfig = await response.json();
}

// Load user profile template
async loadUserProfile() {
    // Try sandbox/user/ first, fall back to core/data/
    let response = await fetch('../../sandbox/user/font-profile-template.json');
    if (!response.ok) {
        response = await fetch('../../core/data/font-profile-template.json');
    }
...
2. **Load Template**: Click "Load Profile" button
   - Loads `core/data/font-profile-template.json`
...
4. **Save Profile**: Click "Save Profile" button
   ...
   - Move to `sandbox/user/` directory

5. **Apply Profile**: Update `sandbox/user/USER.UDT`:
   ```json
   {
       "system": {
           "font_profile": "sandbox/user/font-profile-my-retro-setup.json"
       }
   }
   ```
...
- `core/data/font-system.json` - Master configuration
- `core/data/palette.json` - Legacy color palette (deprecated in favor of font-system.json)
- `core/data/font-profile-template.json` - User profile template
...
- `sandbox/user/font-profile-template.json` - Working copy of template
- `sandbox/user/font-profile-*.json` - Custom user profiles
- `sandbox/user/USER.UDT` - User settings (includes `font_profile` path)
...
**Solution:** Verify `core/data/font-system.json` exists. Reinstall from git:
```bash
git checkout core/data/font-system.json
```

### Profile Template Missing

**Error:** `Font profile not found. Please ensure template exists in core/data/`

**Solution:** Create template from git:
```bash
git checkout core/data/font-profile-template.json
```
...
4. Consult `core/data/font-system.json` for configuration details
