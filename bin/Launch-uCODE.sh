...
health_checklist() {
    ...
    echo "- If Wizard fails: run ./bin/Launch-uCODE.sh wizard server"
...
start_wizard_if_needed() {
    ...
    echo "Wizard server did not start. Try: ./bin/Launch-uCODE.sh wizard server"
    return 1
}
...
case "$component" in
    ...
    --rebuild)
        ...
        ;;
    *)
        echo "Usage: ./Launch-uCODE.sh [core|wizard|goblin] [mode] [--rebuild] [--quiet] [--tty] [--no-log]" >&2
        exit 1
        ;;
esac
