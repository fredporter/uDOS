...
# Conf# Initialize role environment
...
}S_ROOT="${UDOS_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"
export UDOS_VERSION="1.3.1"
export UDOS_UI_PORT="${UDOS_UI_PORT:-8080}"
export UDOS_SERVER_PORT="${UDOS_SERVER_PORT:-8081}"
...
open_ui_omniview() {
    local ui_url="http://localhost:${UDOS_UI_PORT}?role=${ROLE}&level=${UDOS_ACCESS_LEVEL}"
