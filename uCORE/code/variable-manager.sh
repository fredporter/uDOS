...
execute_story() {
    ...
    for ((i=0; i<var_count; i++)); do
        ...
        # Collect variable value using smart input
        local var_value=""
        if [[ -n "$var_values" && "$var_values" != "" ]]; then
            # Use selection for predefined values
            var_value=$("$SCRIPT_DIR/smart-input/smart-input-enhanced.sh" SELECT "$var_prompt" "$var_values" "false" "$var_required")
        else
            # Use prompt for free text
            var_value=$("$SCRIPT_DIR/smart-input/smart-input-enhanced.sh" PROMPT "$var_prompt" "$var_type" "" "$var_required" "$var_help")
        fi
