#!/bin/bash
# uDOS Move Logging System v2.1.0
...
# File paths
DAILY_LOG="${UMEMORY}/daily-log-${TODAY}.md"
MOVE_LOG="${UMEMORY}/moves-${TODAY}.md"
STATS_FILE="${UMEMORY}/stats-${TODAY}.json"
ANALYTICS_FILE="${UMEMORY}/analytics-${TODAY}.json"
ERROR_LOG="${UMEMORY}/errors-${TODAY}.log"
...
---
*Generated by uDOS Logging System v2.1.0*
EOF
    fi
...
archive_old_files() {
    ...
    find "${UMEMORY}" -name "daily-log-*.md" -mtime +7 -exec mv {} "$archive_dir/" \; 2>/dev/null || true
    find "${UMEMORY}" -name "stats-*.json" -mtime +7 -exec mv {} "$archive_dir/" \; 2>/dev/null || true
    find "${UMEMORY}" -name "errors-*.log" -mtime +7 -exec mv {} "$archive_dir/" \; 2>/dev/null || true
...
# Show usage
show_usage() {
    echo "uDOS Move Logging System v2.1.0"
    echo
    echo "Usage: $0 [command] [options]"
    echo
    echo "Commands:"
    echo "  move <command> [type] [context] [duration] [success]"
    echo "    Log a move with metadata"
...
mkdir -p "${UMEMORY}" "${UMEMORY}/archive"
