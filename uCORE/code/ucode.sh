...
EOF
    ...
    # Add footer with current statistics and git information
    local git_repo=$(git remote get-url origin 2>/dev/null || echo "Local repository")
    # Sanitize git URL to remove any embedded credentials
    git_repo=$(echo "$git_repo" | sed 's|https://[^@]*@|https://|' | sed 's|git@github.com:|https://github.com/|')
    
    cat >> "$output_file" << EOF

## Quick Stats
- **Directories**: $dir_count | **Files**: $file_count | **Version**: uDOS v1.3

---

*Generated by uDOS Tree Generator v1.3*  
*Repository: $git_repo | Branch: $(git branch --show-current 2>/dev/null || echo "Unknown")*  
*Last Updated: $timestamp*

📖 **For detailed documentation**: See \`docs/uDOS-System-Documentation.md\`  
🏗️  **For system architecture**: See \`README.md\`
EOF
