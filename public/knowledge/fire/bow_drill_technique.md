...
### Bow Drill Fire Starting Sequence

```sequence
Start->Wood: Select dry wood materials
Wood->Prepare: Cut fireboard and spindle
Prepare->Divot: Create divot in fireboard
Divot->Notch: Cut V-notch in fireboard
Notch->Bow: Assemble bow with string
Bow->Position: Wrap string around spindle
Position->Drill: Begin drilling motion
Drill->Friction: Generate friction heat
Friction->Dust: Wood dust accumulates
Dust->Coal: Dust forms glowing coal
Coal->Tinder: Transfer to tinder bundle
Tinder->Blow: Blow gently on coal
Blow->Flame: Tinder ignites
Flame->Fire: Build fire with kindling
```

### Component Preparation Flow

```flow
st=>start: Need bow drill fire
wood=>operation: Find dry softwood
spindle=>operation: Prepare spindle 8-12 inch
board=>operation: Cut fireboard flat
bow=>operation: Find curved bow stick
string=>operation: Attach bowstring
hand=>operation: Prepare handhold
tinder=>operation: Gather tinder bundle
test=>condition: All components ready?
adjust=>operation: Adjust and refine
drill=>end: Begin drilling

st->wood->spindle->board->bow->string->hand->tinder->test
test(yes)->drill
test(no)->adjust->test
```
