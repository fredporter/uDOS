...
ENV UDOS_VERSION={{udos_version}} \
    USER_ROLE={{user_role}} \
    UDOS_HOME={{install_directory}} \
...
RUN if echo "{{install_packages}}" | grep -q "ripgrep"; then apt-get update && apt-get install -y ripgrep; fi \
    && if echo "{{install_packages}}" | grep -q "fd"; then apt-get update && apt-get install -y fd-find && ln -sf /usr/bin/fdfind /usr/local/bin/fd; fi \
    && if echo "{{install_packages}}" | grep -q "bat"; then apt-get update && apt-get install -y bat && ln -sf /usr/bin/batcat /usr/local/bin/bat; fi \
    && rm -rf /var/lib/apt/lists/*
...
RUN git clone https://github.com/fredporter/uDOS.git {{install_directory}} \
    && cd {{install_directory}} \
    && chmod +x uCode/*.sh \
    && chmod +x uCode/packages/*.sh

# Setup user configuration
RUN mkdir -p {{install_directory}}/uMemory/config \
    && echo '{ \
        "role": "{{user_role}}", \
        "username": "udos", \
        "hostname": "container", \
        ...
        "architecture": "{{architecture}}", \
        "container_mode": true \
    }' > {{install_directory}}/uMemory/config/user.json

# Configure shell
RUN echo 'export UDOS_HOME="{{install_directory}}"' >> ~/.zshrc \
...
RUN if [ "{{enable_chester}}" = "true" ]; then \
        cd {{install_directory}} && ./uCode/packages/install-gemini.sh; \
    fi
...
RUN echo '#!/bin/zsh' > /home/udos/start-udos.sh \
    && echo 'cd {{install_directory}}' >> /home/udos/start-udos.sh \
    && echo 'echo "🌀 Starting uDOS Container..."' >> /home/udos/start-udos.sh \
    && echo 'echo "User Role: {{user_role}}"' >> /home/udos/start-udos.sh \
    && echo 'echo "Access via: http://localhost:8080"' >> /home/udos/start-udos.sh \
    && echo 'code-server --bind-addr 0.0.0.0:8080 --auth none {{install_directory}} &' >> /home/udos/start-udos.sh \
    && echo 'if [ "{{enable_chester}}" = "true" ]; then' >> /home/udos/start-udos.sh \
...
services:
  udos:
    ...
    ports:
      - "{{port_mapping}}"
    volumes:
      - udos-data:{{install_directory}}/uMemory
...
run_container() {
    ...
    docker run -d \
        --name "$CONTAINER_NAME" \
        -p "$PORT_MAPPING" \
        -v udos-data:{{install_directory}}/uMemory \
...
- `{{install_directory}}` - uDOS installation path in container
- `{{user_role}}` - User role configuration
- `{{enable_chester}}` - Chester AI companion enabled
