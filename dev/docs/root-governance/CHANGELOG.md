...
- Installation documentation
  - Comprehensive installation guide at `docs/INSTALLATION.md`
  - Updated `QUICK-START.md` with installation instructions
  - `bin/README.md` documenting installer scripts
  - Updated main `README.md` to reference automated installer

### Changed

- Separated core requirements from wizard requirements in installation flow
- Wizard components now support lazy-loading installation
- Environment setup now auto-generates `WIZARD_ADMIN_TOKEN` and `WIZARD_KEY`

## [1.4.6] - 2026-02-22

### Added

- Supported CI test matrix by profile (`core`, `wizard`, `full`) with dedicated test scopes.

### Changed

- Version surfaces updated to `1.4.6` across package/runtime/distribution metadata.
- Pytest collection is now explicitly scoped via `testpaths` to prevent accidental external tree discovery.

### Fixed

- MCP FastMCP import compatibility for wizard integration tests across MCP package layouts.
- Snapshot baselines revalidated for current UI/test output.
