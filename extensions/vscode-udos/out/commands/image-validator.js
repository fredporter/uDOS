...
exports.previewASCII = previewASCII;
exports.validateTeletext = validateTeletext;
const vscode = __importStar(require("vscode"));
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
async function previewSVG(uri) {
    try {
        const svgContent = fs.readFileSync(uri.fsPath, 'utf-8');
        const report = validateSVG(svgContent);
        const panel = vscode.window.createWebviewPanel('svgPreview', `SVG: ${path.basename(uri.fsPath)}`, vscode.ViewColumn.Two, { enableScripts: true });
...
async function previewASCII() {
    ...
    const panel = vscode.window.createWebviewPanel('asciiPreview', 'ASCII Art Preview', vscode.ViewColumn.Two, { enableScripts: true });
...
async function validateTeletext() {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        vscode.window.showErrorMessage('No file active');
        return;
    ...
    const panel = vscode.window.createWebviewPanel('teletextValidation', `Teletext: ${path.basename(editor.document.fileName)}`, vscode.ViewColumn.Two, { enableScripts: true });
...
function validateASCII(content) {
    const issues = [];
...
function validateTeletextFormat(content) {
    const issues = [];
    const lines = content.split('\n');
...
function getSVGPreviewHTML(svgContent, fileName, report) {
...
function getASCIIPreviewHTML(content, report) {
    const statusColor = report.valid ? '#4ade80' : '#f87171';
...
function getTeletextValidationHTML(content, report) {
    const statusColor = report.valid ? '#4ade80' : '#f87171';
