    ...
    try:
        ...
        if platform.system() == "Darwin":
            ...
            tell application "Terminal"
                activate
                do script "cd {udos_path} && UV_PROJECT_ENVIRONMENT=.venv ./bin/ucode tui -c L"
            end tell
            """
            ...
            for term in terminals:
                try:
                    cmd = f"cd {udos_path} && UV_PROJECT_ENVIRONMENT=.venv ./bin/ucode tui -c L"
                ...
                return jsonify(
                    {
                        "status": "manual",
                        "command": f"cd {udos_path} && ./bin/ucode tui -c L",
        ...
        return jsonify(
            {
                "status": "manual",
                "command": f"cd {project_root} && ./bin/ucode tui -c L",
                "error": str(e),
            }
        )
