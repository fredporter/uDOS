...
class ScrewdriverProvisioner:
    ...
    def provision_device(
        ...
        self.device_manager.update_firmware_status(
            device_id,
            status=FirmwareStatus.OUTDATED  # Mark as updating
        )
        ...
        self.flash_manager.update_flash_progress(
            device_id,
            FlashStage.HEALTH_CHECK,
            95
        )
        ...
        if not post_health.is_healthy() and not force:
            ...
            self.device_manager.update_firmware_status(
                device_id,
                status=FirmwareStatus.INCOMPATIBLE
            )
    ...
    def execute_provision_job(self, job_id: str) -> ProvisionJob:
        ...
        ordered_devices = self._order_devices_by_strategy(
            job.device_ids,
            job.strategy
        )
        
        print(f"Executing {job.strategy.value} provisioning: {len(ordered_devices)} devices")
        print()
...
def demo_screwdriver_provisioner():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        device_mgr.update_device_status("D1", status=DeviceStatus.ONLINE, signal=75, uptime=24.0, msgs_per_sec=145)
        device_mgr.update_device_status("D2", status=DeviceStatus.ONLINE, signal=85, uptime=48.0, msgs_per_sec=267)
        device_mgr.update_device_status("D3", status=DeviceStatus.ONLINE, signal=65, uptime=12.0, msgs_per_sec=89)
