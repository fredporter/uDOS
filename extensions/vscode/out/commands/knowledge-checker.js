...
exports.checkKnowledgeQuality = checkKnowledgeQuality;
const vscode = __importStar(require("vscode"));
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
async function checkKnowledgeQuality(context) {
    ...
    await vscode.window.withProgress({
        ...
        const panel = vscode.window.createWebviewPanel('knowledgeQuality', 'Knowledge Quality Report', vscode.ViewColumn.One, { enableScripts: true });
        ...
        if (report.flaggedForRegen.length > 0) {
            const shouldGenerate = await vscode.window.showInformationMessage(`Found ${report.flaggedForRegen.length} guides that need regeneration. Generate REGEN script?`, 'Yes', 'No');
            if (shouldGenerate === 'Yes') {
                await generateRegenScript(workspaceFolder.uri.fsPath, report.flaggedForRegen);
            }
        }
        vscode.window.showInformationMessage(`✅ Quality check complete: ${report.scannedGuides} guides scanned, ${report.issues.length} issues found`);
    });
}
async function scanKnowledgeBase(knowledgeRoot) {
    const guides = [];
...
async function analyzeQuality(guides, progressCallback) {
    const issues = [];
    const categories = {};
    ...
    const flaggedForRegen = [...new Set(issues
...
async function generateRegenScript(workspaceRoot, flaggedGuides) {
    const commands = [
...
function getQualityReportHTML(report) {
...
//# sourceMappingURL=knowledge-checker.js.map