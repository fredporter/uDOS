    ...
    try {
        ...
        if (response.success) {
            vscode.window.showInformationMessage(
                `✅ Script executed successfully${showExecutionTime ? ` (${executionTime.toFixed(2)}s)` : ''}`
            );
        } else {
...
export async function runInSandbox(context: vscode.ExtensionContext): Promise<void> {
    ...
    try {
        ...
        // Update panel with results
        panel.webview.html = getSandboxPanelHTML(
            response.output,
            scriptName,
            sandboxId,
            response.success,
        ...
        if (response.success) {
            vscode.window.showInformationMessage(
                `🧪 Sandbox execution successful (${executionTime.toFixed(2)}s)`
            );
        } else {
...
async function executeScript(apiUrl: string, script: string, isolated: boolean): Promise<ExecutionResult> {
    try {
...
function cleanupSandbox(sandboxId: string): void {
