...
api_logger.info("Log sink: memory/logs/udos/wizard/*.jsonl")
api_logger.info("=" * 70)
    ...
    if request.method in ["POST", "PUT", "PATCH"]:
        try:
            ...
            if data:
                api_logger.debug(
                    "Request payload",
                    ctx={"correlation_id": correlation_id, "request_payload": data},
                )
        except Exception:
            pass
...
def log_response(response):
    ...
    if correlation_id:
        ...
        api_logger.debug(
            "Response sent",
            ctx={"path": request.path, "status_code": response.status_code},
        )
...
def handle_error(error):
    ...
    api_logger.error(
        "Unhandled API error",
        ctx={"correlation_id": correlation_id, "path": request.path},
        err=error,
        exc_info=True,
    )
    ...
    return jsonify(
        {
            ...
            "documentation": "https://github.com/fredporter/uDOS/tree/main/dev/docs",
