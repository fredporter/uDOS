...
empty_trash() {
    ...
    read -p "Type 'EMPTY TRASH' to confirm permanent deletion: " confirmation
    
    if [[ "$confirmation" == "EMPTY TRASH" ]]; then
        echo -e "${YELLOW}🗑️  Emptying trash...${NC}"
        ...
        log_action "EMPTIED_TRASH" "All items permanently deleted"
        echo -e "${GREEN}✅ Trash emptied successfully!${NC}"
    else
        echo -e "${BLUE}❌ Trash emptying cancelled${NC}"
    fi
}
