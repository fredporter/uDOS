<script lang="ts">
	...
	onMount(async () => {
		...
		// Tauri event listeners for file opening from uDOS
		try {
			const { listen } = await import("@tauri-apps/api/event");
			const { invoke } = await import("@tauri-apps/api/core");

			console.log("[Typo] Setting up event listeners...");

			// Listen for diagnostics
			await listen("typo-diagnostics", (event: any) => {
				console.log("[Typo] DIAGNOSTICS:", event.payload);
				alert("[Typo] Diagnostics received: " + event.payload);
			});

			// Listen for open-file events from Rust backend
			await listen("open-file", async (event: any) => {
				const filepath = event.payload;
				console.log("[Typo] Received open-file event:", filepath);
				alert("[Typo] Received file: " + filepath);

				try {
					// Read the file using Tauri command
					const fileContent = await invoke("read_markdown_file", {
						path: filepath,
					});
					...
					console.log("[Typo] File loaded successfully:", filepath);
					alert("[Typo] File loaded: " + filepath);
				} catch (error) {
					console.error("[Typo] Failed to open file:", error);
					alert("[Typo] Failed to open file: " + error);
				}
			});

			// Get diagnostics on load
			try {
				const diagnostics = await invoke("get_queue_diagnostics");
				console.log("[Typo] Queue Diagnostics:", diagnostics);
			} catch (error) {
				console.error("[Typo] Failed to get diagnostics:", error);
			}

			console.log("[Typo] Tauri event listeners registered");
			alert("[Typo] Event listeners registered successfully");
		} catch (e) {
			// Not running in Tauri, ignore
			console.log("[Typo] Not running in Tauri environment:", e);
			alert("[Typo] Not in Tauri environment: " + e);
		}
	});
</script>
