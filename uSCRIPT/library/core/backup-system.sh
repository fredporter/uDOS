...
UNDO_STACK_FILE="$UMEMORY_DIR/system/undo-stack.json"
BACKUP_METADATA="$UMEMORY_DIR/system/backup-metadata.json"

# Ensure system directories exist
mkdir -p "$UMEMORY_DIR/system" "$UMEMORY_DIR/backups/session" "$UMEMORY_DIR/backups/daily" "$UMEMORY_DIR/backups/manual"
...
# Get backup directory based on role
get_backup_dir() {
    local role="${1:-$(get_user_role)}"
    case "$role" in
        tomb|sorcerer|wizard|imp|drone|ghost) echo "$UDOS_ROOT/$role/backup" ;;
        *) echo "$UDOS_ROOT/ghost/backup" ;;
    esac
}
...
create_backup() {
    ...
    if tar -czf "$backup_file" \
        ...
        --exclude="uMEMORY/backups" \
        --exclude="cache/*" \
        uMEMORY sandbox wizard 2>/dev/null; then
    ...
    # Log the backup as a move
    log_move "backup" "$description" "{\"type\":\"$backup_type\",\"file\":\"$final_file\",\"size\":$backup_size}"
...
cleanup_old_backups() {
    ...
    # Remove files not in keep list, move to trash
    find "$backup_dir" -name "*-backup.tar.gz*" -type f | while read -r file; do
        if ! echo "$files_to_keep" | grep -Fxq "$file"; then
            ...
            if [ -x "$UDOS_ROOT/uCORE/bin/trash" ]; then
                "$UDOS_ROOT/uCORE/bin/trash" file "$file" backups
                log_info "Moved old backup to trash: $(basename "$file")"
            else
                rm -f "$file"
