...
1. **Fork and clone the repository**
   ```bash
   # Clone WITH development tools (recommended for contributors)
   git clone --recurse-submodules https://github.com/YOUR_USERNAME/uDOS.git
   cd uDOS
   ...
   > **Note**: The `--recurse-submodules` flag includes the `dev/` folder with development tools (roadmap, sessions, utilities). Without it, you'll only get the core 1,331 files.
...
## Working with Dev Mode Submodule

The `dev/` folder is a **git submodule** pointing to [uDOS-dev](https://github.com/fredporter/uDOS-dev) repository. It contains development tools, roadmap, and session logs.

### Making Changes to dev/

```bash
# Navigate into dev/ submodule
cd dev/

# Make your changes to tools, roadmap, etc.
# Commit changes within the submodule
git add .
git commit -m "Update development tool X"
git push origin main

# Return to main repo and update submodule reference
cd ..
git add dev
git commit -m "Update dev/ submodule reference"
git push
```

### Pulling Latest dev/ Changes

```bash
# Update submodule to latest commit
git submodule update --remote dev

# Commit the updated reference in main repo
git add dev
git commit -m "Update dev/ to latest"
```

> **Note**: Changes to `dev/` require TWO commits:
> 1. Commit in the `uDOS-dev` repository (inside dev/ folder)
> 2. Commit the submodule reference in the main `uDOS` repository
...
# SHAKEDOWN comprehensive validation (v1.2.12+)
./start_udos.sh
> SHAKEDOWN                    # Run all tests (142/148 passing)
> SHAKEDOWN --verbose          # Detailed output
> SHAKEDOWN --quick            # Core tests only
> SHAKEDOWN --report           # Generate JSON report

# All uPY tests (v1.2.x structure)
pytest memory/ucode/tests/ -v

# Specific test categories
pytest memory/ucode/tests/test_upy_*.py        # uPY runtime tests
pytest memory/tests/test_v1_1_18_*.py          # v1.1.18 archive tests
pytest memory/tests/integration/ -v            # Integration tests

# With coverage
pytest memory/ucode/tests/ --cov=core --cov-report=html

# Specific test file
pytest memory/ucode/tests/test_upy_math.py -v

# Specific test function
pytest memory/ucode/tests/test_upy_math.py::TestMathOperations::test_addition -v
```

### SHAKEDOWN Test Coverage (v1.2.12)
